...
describe('Элементы интерфейса', () => {
  ...
  it('По кнопке "Каталог" открывается меню с разделами', () => {
    cy.get('.header__bars').click()
    cy.get('.mob_menu').should('be.visible')
  })

  it('При нажатии на + на разделе открываются списки подразделов', () => {
    cy.get('.header__bars').click()
    cy.get('.mainCatalog > :nth-child(1) > .razdel > .openCatalog').click()
    cy.get(':nth-child(1) > .header__catalog__submenu > .row > :nth-child(1)').should('be.visible')
  })
  ...
  it('Успешная отправка формы с заполненными полями из хедера', () => {
    ...
    cy.get('.header__bars').click()
    cy.get('.mob_menu__callback_btn').click()
  ...
  it.skip('Открытие всплывающего окна при нажатии на поисковую строку', () => {
    cy.get('#edit-search-theme-form-2').click()
    cy.get('#digi-shield').should('be.visible')
  })

  it.skip('Закрытие поискового окна по кнопке "закрыть"', () => {
    cy.get('#edit-search-theme-form-2').click()
...
describe('Избранное', () => {
  ...
  it('Нажатие на звездочку на товаре в карточке добавляет товар в раздел избранного', () => {
    ...
    cy.get('#favourite-count').click()
    cy.get('.shortcard.collect').should('be.visible')
  })
  ...
  it('Добавление коллекции в избранное', () => {
    ...
    cy.get('.card__manage_btn > .svg-ico').click()
   
    cy.get('.noty_body').should('contain', 'Добавили в избранное')

    cy.get('#favourite-count').click()
    cy.get('#favourite_collects_title').should('be.visible')
    cy.get('#favourite_goods').should('exist')
  })
  ...
  it('Нажатие на звездочку на странице избранного удаляет товар из раздела', () => {
    ...
    cy.get('.card__manage_btn').click()
    cy.get('#favourite-count').click()
    cy.get('.shortcard__manage_btn').click()
    cy.reload()
...
describe('Листинг', () => {
  ...
  it('Выбор и снятие чекбоксов в "разделе"', () => {
    cy.visit('https://www.antica.su/razdel/bezhevaya-napolnaya-keramicheskaya-plitka')
    cy.get('#good_button').click()
    cy.get('.catalog__filter__toggle_btn').click()
